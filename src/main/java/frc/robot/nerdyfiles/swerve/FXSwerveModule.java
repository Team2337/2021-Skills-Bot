 ...
 * swerve module uses joystick values from the command to change the angle and
 * drive positions This Object uses the TalonFX's for both the angle motor and
 * drive motor Both will need to be passed in when the object is created
 * 
...
public class FXSwerveModule {
    ...
    /**
     * Proportional value for the drive motor speed This is used to scale the error
     * to a funcitonal speed for the motors
     */
    private static final double kDriveP = 0.1;

    /**
     * Integral value for the drive motor speed This value is used to reduce
     * oscillation when sending the motor to a setpoint
     */
    private static final double kDriveI = 0;

    /**
     * Derivative value for the drive motor speed This is added to the speed of the
     * motors to increase power at smaller errors
     */
    private static final double kDriveD = 0;

    /**
     * Feet Forward value for the drive motor speed Sets the error to be higher than
     * the actual error causing the motor to increase the power output to be able to
     * reach its setpoint
     */
    ...
    // Inches traveled per tick can be found by using our circumference (2pi * r) of
    // our wheel
     ...
     * 
     * @see SwerveDriveCommand
     * @param position                - int Module ID to call each module (not CAN
     *                                IDs)
     * @param driveMotor              - TalonFX motor object with CAN ID of the
     *                                module's drive motor
     * @param angleMotor              - TalonFX motor object with CAN ID of the
     *                                module's angle motor
     * @param canCoder                - CANCoder sensor object that measures the
     *                                rotation of the angle motor
     * @param angleMotorOffsetDegrees - The angle offset for the current module in
     *                                degrees
     */
    public FXSwerveModule(Constants.Swerve.ModulePosition position, TalonFX driveMotor, TalonFX angleMotor,
            CANCoder canCoder, double angleMotorOffsetDegrees) {
        ...
        // canCoderConfiguration.magnetOffsetDegrees = canCoder.configGetCustomParam(0)
        // + (position.value * 90);
        ...
        // driveTalonFXConfiguration.closedloopRamp = 0.2; // 0.2s seconds from neutral
        // to full output
        ...
        // Sets the minimum current reading needed in order to initiate the current
        // reading
        currentLimitConfigurationAngle.triggerThresholdCurrent = 40;
        // Sets the minimum amount of time beyond the trigger threshold reading needed
        // in order to initiate the current reading
     ...
     * 
     * @param desiredState      - A SwerveModuleState representing the desired new
     *                          state of the module
     * @param shouldUpdateangle - is false if all joystick inputs are zero, so that
     *                          modules dont turn back to a zero angle when
     *                          releasing joystick
     * @param isJoystickControl - Use PercentOutput (intended/Max Velocity) if
     *                          isJoyStickControl == true, otherwise use 'set'
     *                          Velocity.
     */
    public void setDesiredState(SwerveModuleState desiredState, boolean shouldUpdateAngle, boolean isJoystickControl, double attemptAntiSKid) {
        ...
        if (isJoystickControl) {
            driveMotor.set(TalonFXControlMode.PercentOutput,
                    feetPerSecond / Constants.Swerve.MAX_FEET_PER_SECOND * attemptAntiSKid);
        } else {
    ...
    private void setAngle(Rotation2d angle) {
        ...
        // Find the rotational difference between the current state and the desired
        // state
        ...
        // Set the absolute position for the motor by converting our degrees to # of
        // ticks for the rotational value
